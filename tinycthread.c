...
int mtx_timedlock(mtx_t *mtx, const struct timespec *ts)
{
  ...
  {
    timeoutMs  = (DWORD)(ts->tv_sec  - current_ts.tv_sec)  * 1000; //modified, added cast
...
int cnd_timedwait(cnd_t *cond, mtx_t *mtx, const struct timespec *ts)
{
  ...
  if (timespec_get(&now, TIME_UTC) == 0)
  {
    DWORD delta = (DWORD)((ts->tv_sec - now.tv_sec) * 1000 +
      (ts->tv_nsec - now.tv_nsec + 500000) / 1000000); //modified, added cast
...
int thrd_sleep(const struct timespec *duration, struct timespec *remaining)
{
  ...
  t = SleepEx((DWORD)(duration->tv_sec * 1000 +
    ...
    (((duration->tv_nsec % 1000000) == 0) ? 0 : 1)), //modified, added cast
              TRUE);
