...
int compare(const void *a,const void *b,const void *config) {
  return strncmp((char*)a,(char*)b,255);
}

bool insert(struct rbtree *rb,const char *k,int v) {
  struct Pair *p;
  p=(struct Pair*)rbfind(k,rb);
...
int main() {
  ...
  //
  rblist=rbopenlist(rb);

  while((p=(struct Pair*)rbreadlist(rblist))) {
    printf("%s = %i\n",p->k,p->v);
  }
